...
const HTML_TEMPLATE = path.join(RESOURCES, 'index.template.html');

const HTML = path.join(RESOURCES, 'index.html');

// XXX: This is a workaround for the dev mode only where the file might not be
// present in the src/app yet. In production build it will be always there,
// generated by the pre-compilation step of hadron-build. This is all temporary
// until we get rid of the hadron-style-manager
//
// Making sure that main html file exists if it wasn't before
try {
  // eslint-disable-next-line no-sync
  fs.statSync(HTML);
} catch (e) {
  // eslint-disable-next-line no-sync
  fs.writeFileSync(HTML, fs.readFileSync(HTML_TEMPLATE));
}
